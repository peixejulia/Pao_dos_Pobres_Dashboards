...
with st.expander("ℹ️ Como ler este gráfico"):
    st.markdown(
        ...
        "**O que cada cor significa:** dentro de cada fatia (mês), os anos aparecem "
        f"**empilhados** um sobre o outro, do centro para fora — o anel mais claro (perto "
        f"do centro) é **{min(ANOS)}**, e o mais escuro (na borda externa) é **{max(ANOS)}** "
        "(veja a legenda embaixo do gráfico). Ou seja, **a fatia inteira de um mês soma "
        f"todos os anos**: a borda externa representa o total daquele mês somando "
        f"{min(ANOS)}–{max(ANOS)}, não o valor de um único ano.\n\n"
        "**Para comparar um ano específico:** olhe a **espessura (largura) do anel** "
        "daquela cor, não até onde ele chega — um ano com mais registros tem um anel mais "
        "grosso, mesmo aparecendo no meio da pilha. Passe o mouse sobre qualquer anel para "
        "ver o valor exato daquele ano naquele mês.\n\n"
        "**O que o tamanho da fatia inteira significa:** quanto mais a fatia se estende "
        "para fora do centro (mais \"comprida\" de ponta a ponta), mais registros houve "
        "naquele mês somando todos os anos exibidos. Os números no eixo (0, 200, 400...) "
        "mostram a escala de volume.\n\n"
        "**A linha vermelha pontilhada** marca a **média geral** do total mensal (soma dos "
        "anos) — meses cuja fatia ultrapassa essa linha tiveram volume acima da média.\n\n"
        "**Dica:** clique nos anos da legenda para remover uma camada do empilhamento e "
        "ver como o total mudaria sem aquele ano; passe o mouse sobre qualquer anel para "
        "ver o valor exato."
    )
